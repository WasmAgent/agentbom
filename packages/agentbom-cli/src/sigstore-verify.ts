...
function parseBundle(raw: string): SigstoreBundle {
  ...
  if (typeof data.mediaType !== "string")
    throw new Error("Invalid bundle: missing mediaType");
  ...
  if (!isRecord(vm))
    throw new Error("Invalid bundle: missing verificationMaterial");
  const vmContent = vm.content;
  if (!isRecord(vmContent))
    throw new Error("Invalid bundle: missing verificationMaterial.content");
    ...
    verificationMaterial: {
      content: {
        ...
        certificates: certs.map((c: unknown) => ({
          rawBytes:
            isRecord(c) && typeof c.rawBytes === "string" ? c.rawBytes : "",
        })),
      },
...
export function verifySigstoreBundle(
  ...
  if (options.bundlePath) {
    try {
      ...
      return {
        ...
        issuerMatch: true,
        errors: [
          `Bundle load error: ${err instanceof Error ? err.message : String(err)}`,
        ],
        warnings: [],
      };
    }
  ...
  try {
    const leafRaw = base64ToBuffer(
      bundle.verificationMaterial.content.certificates[0].rawBytes,
    );
    ...
    const artifact =
      ...
      (options.artifactPath
        ? readFileSync(resolve(options.artifactPath))
        : null);
    ...
    if (artifact) {
      ...
      if (bundle.content.messageDigest && artifact) {
        ...
        const expectedDigest = base64ToBuffer(
          bundle.content.messageDigest.digest,
        );
    ...
    if (options.trustedIssuer) {
      ...
      if (!issuerMatch) {
        errors.push(
          `SSO issuer mismatch: expected "${options.trustedIssuer}" in certificate SANs`,
        );
      }
    }
  } catch (err) {
    certificateValid = false;
    errors.push(
      `Certificate error: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  if (tlogEntries && tlogEntries.length > 0) {
    if (options.offline) {
      warnings.push(
        "Offline mode: transparency log entries present but Rekor check skipped",
      );
      tlogVerified = false;
    } else {
  ...
  if (options.artifactContent || options.artifactPath) {
    try {
      const artifactRaw =
        ...
        (options.artifactPath
          ? readFileSync(resolve(options.artifactPath), "utf-8")
          : "");
...
export function verifySigstoreCommand(args: string[]): number {
  ...
  if (!bundlePath) {
    console.error(
      "Error: passport verify-sigstore requires a <bundle.json> argument",
    );
    return 1;
  }
