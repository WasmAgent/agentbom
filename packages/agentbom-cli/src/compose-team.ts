...
export function composeTeamCommand(args: string[]): number {
  ...
  for (const filePath of args) {
    ...
    try {
      ...
      console.error(
        `Error: Invalid BOM format in file: ${resolvedPath}: not valid JSON`,
      );
      return 1;
    }
    ...
    if (!result.valid) {
      console.error(
        `Error: Invalid BOM format in file: ${resolvedPath}: ${result.errors[0]}`,
      );
      return 1;
    }
  ...
  for (const { data, path } of boms) {
    ...
    const collab = data.agent_collaboration as
