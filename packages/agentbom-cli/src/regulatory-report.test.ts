...
const FULL_BOM = {
  ...
  evidence_layer: {
    ...
    evidence_hashes: [
      { type: "sha256", hash: "abc123", timestamp: "2026-07-01T00:00:00Z" },
    ],
  },
  attestation: {
    generator: "trust-cli",
...
describe("reportCommand", () => {
  describe("argument parsing", () => {
    ...
    it("returns 1 for invalid --format", () => {
      const path = writeTmpFile("bom.json", JSON.stringify(MINIMAL_BOM));
      expect(
        reportCommand([path, "--framework", "soc2", "--format", "pdf"]),
      ).toBe(1);
    });
    ...
    it("returns 1 for non-existent file", () => {
      expect(
        reportCommand(["/nonexistent/bom.json", "--framework", "soc2"]),
      ).toBe(1);
    });
  ...
  describe("SOC2 framework", () => {
    ...
    it("detailed evidence level includes more evidence citations", () => {
      ...
      const detailedReport = JSON.parse(
        detailedCalls[detailedCalls.length - 1],
      );

      // Detailed should have at least as many evidence citations as summary
      expect(
        detailedReport.executive_summary.evidence_citations,
