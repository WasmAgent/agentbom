...
function parseDuration(duration: string): number {
  ...
  if (!match)
    throw new Error(
      `Invalid duration format: "${duration}". Use e.g. 1y, 90d, 6m, 24h`,
    );
...
export async function signPassport(options: SignOptions): Promise<string> {
  ...
  if (!("expires_at" in validity)) {
    const expiryMs = expires
      ? parseDuration(expires)
      : 365 * 24 * 60 * 60 * 1000;
  ...
  if (!structureResult.valid) {
    throw new Error(
      `Invalid passport format: ${structureResult.errors.join("; ")}`,
    );
  }
  ...
  const expiresAtStr =
    typeof validity.expires_at === "string" ? validity.expires_at : undefined;
