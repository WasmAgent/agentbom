...
function writeMinimalJwt(payload: Record<string, unknown>): string {
  ...
  const jwtPath = join(
    tmpDir,
    `passport-${Math.random().toString(36).slice(2)}.jwt`,
  );
...
describe("verifyChain", () => {
  ...
  it("reports signature errors when no key is provided", () => {
    ...
    expect(result.root.errors).toContain(
      "No public key provided; signature not verified",
    );
  ...
  it("reports integrity failure for tampered registry artifacts", () => {
    ...
    expect(result.nodes[0].errors.some((e) => e.includes("integrity"))).toBe(
      true,
    );
    expect(result.valid).toBe(false);
  });
  ...
  it("detects and prevents cycles", () => {
    ...
    writeFileSync(
      aPath,
      JSON.stringify({ ...aBase, dependencies: [{ id: bCasId }] }),
      "utf-8",
    );
    ...
    writeFileSync(
      bPath,
      JSON.stringify({ ...bBase, dependencies: [{ id: aCasId }] }),
      "utf-8",
    );
