...
function soc2Controls(ev: EvidenceLevel): ControlDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "The entity responds to identified incidents to mitigate impact.",
...
function iso27001Controls(ev: EvidenceLevel): ControlDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "Identity management life cycle for people and non-person entities.",
      assess: (bom, profile) => assessIdentityAuth(bom, profile, ev),
    },
    {
      id: "ISO27001.A.8.9",
    ...
    },
    {
      ...
      description:
        "Information security events and weaknesses to be reported and managed.",
      assess: (bom, profile) => assessRiskIncidentResponse(bom, profile, ev),
    },
    {
      id: "ISO27001.A.8.25",
    ...
    },
    {
      ...
      description:
        "Independent review of information security to be performed.",
      assess: (bom, profile) => assessAttestation(bom, profile, ev),
    },
  ];
}
...
function aiActControls(ev: EvidenceLevel): ControlDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "Description of data collection, preparation, and training processes.",
    ...
    },
    {
      ...
      description:
        "Design and development choices for enabling effective oversight of the system.",
    ...
    },
    {
      ...
      description:
        "Risk management system and mitigation measures implemented.",
      assess: (bom, profile) => assessRiskMonitoring(bom, profile, ev),
    },
    {
      ...
      description:
        "Performance metrics for accuracy, robustness, and cybersecurity of the system.",
      assess: (bom, profile) => assessRiskIncidentResponse(bom, profile, ev),
    },
    {
      ...
      description:
        "Evidence of conformity assessment and technical documentation.",
      assess: (bom, profile) => assessAttestation(bom, profile, ev),
    },
  ];
}
...
function assessIdentityAccess(
  ...
  if (!identity) {
    findings.push(
      "Identity section missing — cannot verify agent identity or access controls",
    );
  ...
  if (
    context &&
    (allowedContexts.length === 0 || allowedContexts.includes(context))
  ) {
    evidence.push({
...
function assessIdentityAuth(
  ...
  if (!identity) {
    findings.push(
      "Identity section missing — cannot verify authentication mechanism",
    );
    return { status: "not_satisfied", evidence, findings };
  }
...
function assessToolPermissions(
  ...
  if (!toolLayer || toolLayer.length === 0) {
    findings.push(
      "Tool layer missing or empty — cannot assess permission controls",
    );
    return { status: "not_satisfied", evidence, findings };
  }
  ...
  const blockedPerms = new Set(
    profile.rules.tool_layer?.blocked_permissions ?? [],
  );
  const blockedSources = new Set(
    profile.rules.tool_layer?.blocked_sources ?? [],
  );
  ...
  for (const tool of toolLayer) {
    ...
    const hasBlocked = permissions.some((p) =>
      Array.from(blockedPerms).some((bp) =>
...
function assessRiskMonitoring(
  ...
  const unmitigated: Record<string, number> = {
    critical: 0,
    high: 0,
    medium: 0,
    low: 0,
  ...
  const maxHigh =
    profile.rules.risk_layer?.max_unmitigated_high ?? Number.POSITIVE_INFINITY;
  const maxMedium =
  ...
  if (unmitigated.high > maxHigh) {
    findings.push(
      `${unmitigated.high} unmitigated high risk(s) exceeds threshold (${maxHigh})`,
    );
  }
...
function assessRiskIncidentResponse(
  ...
  if (!auditLog || auditLog.length === 0) {
    findings.push(
      "No audit log entries — cannot verify incident detection and response",
    );
    return { status: "not_satisfied", evidence, findings };
  }
...
function assessChangeManagement(
  ...
  if (!identity) {
    findings.push(
      "Identity section missing — cannot assess versioning and change tracking",
    );
    return { status: "not_satisfied", evidence, findings };
  }
  ...
  const status: ControlObjective["status"] =
    evidence.length > 0 ? "satisfied" : "not_satisfied";
  return { status, evidence, findings };
}
...
function assessAttestation(
  ...
  if (!attestation) {
    if (
      ...
      findings.push(
        "Attestation section missing — required for independent review",
      );
      return { status: "not_satisfied", evidence, findings };
    }
    return { status: "not_applicable", evidence, findings };
...
function assessAIActData(
  ...
  if (toolLayer && toolLayer.length > 0) {
    ...
    findings.push(
      "No tool inventory — data governance scope cannot be assessed",
    );
  }

  // Check if evidence layer exists (AEP references indicate evidence trail)
  const evidenceLayer = bom.evidence_layer as
    | Record<string, unknown>
    | undefined;
  if (evidenceLayer) {
    const aepRefs =
      (evidenceLayer.aep_references as string[] | undefined) ?? [];
    if (aepRefs.length > 0) {
      evidence.push({
...
function assessAIActOversight(
  ...
  if (attestation?.signature) {
    evidence.push({
      source: "agentbom.attestation.signature",
      description:
        "Attestation signature present — supports human oversight chain",
    });
  }
...
function buildReport(
  ...
  const satisfied = controlObjectives.filter(
    (c) => c.status === "satisfied",
  ).length;
  const partial = controlObjectives.filter(
    (c) => c.status === "partial",
  ).length;
  const notSatisfied = controlObjectives.filter(
    (c) => c.status === "not_satisfied",
  ).length;
...
function formatReportText(report: RegulatoryReport): string {
  ...
  lines.push(
    `Report generated by Agent Trust Infrastructure — ${meta.generated_at}`,
  );
  lines.push(
    "════════════════════════════════════════════════════════════════════════════════",
  );
...
export function reportCommand(args: string[]): number {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--help" || args[i] === "-h") {
      ...
      if (val !== "summary" && val !== "detailed") {
        console.error(
          `Error: invalid --evidence-level "${val}"; use summary or detailed`,
        );
        return 1;
      }
  ...
  if (!bomPath) {
    ...
    console.error(
      "Usage: agent-trust report <bom.json> --framework <soc2|iso27001|ai-act>",
    );
    return 1;
  }
